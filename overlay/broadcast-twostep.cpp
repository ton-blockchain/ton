...
void BroadcastsTwostep::send(OverlayImpl *overlay, PublicKeyHash send_as, td::BufferSlice data, td::uint32 flags) {
  ...
  if (data_size >= FEC_MIN_BYTES && other_nodes.size() >= FEC_MIN_OTHER_NODES) {
    ...
    td::Bits256 broadcast_id = get_tl_object_sha_bits256(create_tl_object<ton_api::overlay_broadcastTwostep_id>(
        flags, date, send_as.bits256_value(), data_hash, static_cast<std::int32_t>(part_size)));
    auto R = td::raptorq::Encoder::create(part_size, std::move(data));
    if (R.is_error()) {
    ...
    td::Bits256 broadcast_id = get_tl_object_sha_bits256(create_tl_object<ton_api::overlay_broadcastTwostep_id>(
        flags, date, send_as.bits256_value(), data_hash, static_cast<std::int32_t>(data_size)));
...
static bool handle_error(td::Result<std::pair<td::BufferSlice, PublicKey>> &&R) {
  if (R.is_error()) {
    td::Status reason = R.move_as_error();
    if (reason.code() == ErrorCode::notready) {
      LOG(DEBUG) << "failed to send twostep broadcast: " << reason;
    } else {
      LOG(WARNING) << "failed to send twostep broadcast: " << reason;
    }
    return true;
  }
  return false;
...
void BroadcastsTwostep::signed_simple(OverlayImpl *overlay, BroadcastTwostepDataSimple &&data,
  ...
  td::BufferSlice broadcast = create_serialize_tl_object<ton_api::overlay_broadcastTwostepSimple>(
      data.flags, data.date, V.second.tl(), cert ? cert->tl() : Certificate::empty_tl(), std::move(data.data),
      std::move(V.first));
  for (auto &dst : data.dsts) {
    td::actor::send_closure(overlay->overlay_manager(), &Overlays::send_message_via, dst, data.src,
...
void BroadcastsTwostep::signed_fec(OverlayImpl *overlay, BroadcastTwostepDataFec &&data,
  ...
  td::BufferSlice broadcast = create_serialize_tl_object<ton_api::overlay_broadcastTwostepFec>(
      data.flags, data.date, V.second.tl(), cert ? cert->tl() : Certificate::empty_tl(), data.data_hash, data.data_size,
      data.seqno, std::move(data.part), std::move(V.first));
  td::actor::send_closure(overlay->overlay_manager(), &Overlays::send_message_via, data.dst, data.src,
...
static void rebroadcast(OverlayImpl *overlay, const PublicKeyHash &src, const td::BufferSlice &data) {
  adnl::AdnlNodeIdShort addr(src);
  overlay->iterate_all_peers([&](const adnl::AdnlNodeIdShort &key, OverlayPeer &peer) {
    if (key != addr) {
      td::actor::send_closure(overlay->overlay_manager(), &Overlays::send_message_via, key, addr, overlay->overlay_id(),
                              data.clone(), overlay->rldp2());
    }
  });
}
...
td::Status BroadcastsTwostep::process_broadcast(OverlayImpl *overlay, adnl::AdnlNodeIdShort src_peer_id,
  ...
  td::Bits256 broadcast_id = get_tl_object_sha_bits256(create_tl_object<ton_api::overlay_broadcastTwostep_id>(
      broadcast->flags_, broadcast->date_, src_keyhash.bits256_value(), sha256_bits256(broadcast->data_.as_slice()),
  ...
  if (src_peer_id == adnl::AdnlNodeIdShort(src_keyhash)) {
    rebroadcast(overlay, src_keyhash, serialize_tl_object(broadcast, true));
  }
...
td::Status BroadcastsTwostep::process_broadcast(OverlayImpl *overlay, adnl::AdnlNodeIdShort src_peer_id,
  ...
  td::Bits256 broadcast_id = get_tl_object_sha_bits256(create_tl_object<ton_api::overlay_broadcastTwostep_id>(
      broadcast->flags_, broadcast->date_, src_keyhash.bits256_value(), broadcast->data_hash_,
      static_cast<td::int32>(part_size)));
  td::BufferSlice to_sign = create_serialize_tl_object<ton_api::overlay_broadcastTwostepFec_toSign>(
