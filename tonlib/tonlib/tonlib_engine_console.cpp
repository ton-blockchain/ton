...
void const *tonlib_event_loop_wait(TonlibEventLoop *loop, double timeout) {
...
void tonlib_response_await_suspend(TonlibResponse *response, void const *continuation) {
...
char const *tonlib_response_get_error_message(TonlibResponse *response) {
  return response->result().error().message().data();
}

char const *tonlib_response_get_response(TonlibResponse *response) {
...
td::Result<tonlib::FFIEngineConsoleClient> create_ffi_client(TonlibEventLoop *loop, char const *config) {
...
td::Result<ton::tl_object_ptr<ton::ton_api::Function>> parse_query(char const *query) {
...
TonlibEngineConsole *tonlib_engine_console_create(TonlibEventLoop *loop, char const *config) {
...
char const *tonlib_engine_console_get_error_message(TonlibEngineConsole *console) {
  return console->client.error().message().data();
}

TonlibResponse *tonlib_engine_console_request(TonlibEngineConsole *console, char const *query) {
