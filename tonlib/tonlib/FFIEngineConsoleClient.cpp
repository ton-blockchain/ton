...
FFIEngineConsoleClient::FFIEngineConsoleClient(FFIEventLoop& loop, td::IPAddress address, ton::PublicKey public_key,
    ...
    : loop_(loop) {
  std::promise<td::actor::ActorOwn<ClientWrapper>> wrapper_promise;

  loop_.run_in_context([&] {
    wrapper_promise.set_value(td::actor::create_actor<ClientWrapper>("EngineConsoleClient", address, public_key,
                                                                     private_key, loop.new_actor()));
  });

  client_ = wrapper_promise.get_future().get();
}
