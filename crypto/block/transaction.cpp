...
/**
 * Initializes account_storage_stat of the account from scratch.
 ...
 */
td::Status Account::compute_account_storage_stat() {
  if (storage.is_null()) {
  ...
  AccountStorageStat& stat = account_storage_stat.value_force();
  TRY_STATUS(stat.replace_roots(storage->prefetch_all_refs()).move_as_status());
  ...
  if (storage_dict_hash && storage_dict_hash.value() != stat.get_dict_hash()) {
...
td::Status Account::init_account_storage_stat(Ref<vm::Cell> dict_root) {
  ...
  account_storage_stat = AccountStorageStat(std::move(dict_root), storage->prefetch_all_refs(), storage_used.cells - 1,
                                            storage_used.bits - storage->size());
  if (storage_dict_hash && storage_dict_hash.value() != account_storage_stat.value().get_dict_hash()) {
    return td::Status::Error(PSTRING() << "invalid storage dict hash: computed "
                                       << account_storage_stat.value().get_dict_hash().to_hex() << ", found "
...
bool Transaction::compute_state(const SerializeConfig& cfg) {
  ...
  if (account.storage.is_null() || new_storage->size_refs() != account.storage->size_refs()) {
    storage_refs_changed = true;
  } else {
    for (unsigned i = 0; i < new_storage->size_refs(); i++) {
      if (new_storage->prefetch_ref(i)->get_hash() != account.storage->prefetch_ref(i)->get_hash()) {
        storage_refs_changed = true;
        break;
      }
    }
  ...
  if (storage_refs_changed || (cfg.store_storage_dict_hash && !account.storage_dict_hash)) {
    ...
    new_storage_used.bits -= account.storage->size();
    new_storage_used.bits += new_storage->size();
 ...
 * @param serialize_cfg Pointer to store the serialization configuration.
...
td::Status FetchConfigParams::fetch_config_params(
  ...
  *old_mparams = config.get_config_param(9);
  ...
  {
    serialize_cfg->store_storage_dict_hash = config.get_global_version() >= 10;
  }
  {
