...
class PrunnedCell final : public Cell {
 public:
  ...
  static td::Result<Ref<PrunnedCell<ExtraT>>> create(const PrunnedCellInfo& prunned_cell_info, ExtraT&& extra) {
    return create([](size_t bytes) { return ::operator new(bytes); }, true, prunned_cell_info, std::move(extra));
  }

  static td::Result<Ref<PrunnedCell<ExtraT>>> create(auto&& allocator, bool should_free,
