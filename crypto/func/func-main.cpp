...
int main(int argc, char* argv[]) {
  ...
  p.add_option('I', "interactive", "Enable interactive mode (parse stdin)",
               []() { funC::interactive = true; });
  p.add_option('o', "output", "Write generated code into specified file instead of stdout",
               [&](td::Slice arg) { output_filename = arg.str(); });
  p.add_option('v', "verbose", "Increase verbosity level (extra information output into stderr)",
               []() { ++funC::verbosity; });
  p.add_checked_option('i', "indent", "Sets indentation for the output code (in two-space units)",
                       [](td::Slice arg) -> td::Status {
                         TRY_RESULT_ASSIGN(funC::indent, td::to_integer_safe<int>(arg));
                         funC::indent = std::max(0, funC::indent);
                         return td::Status::OK();
                       });
  p.add_option('A', "asm-preamble", "Prefix code with `\"Asm.fif\" include` preamble",
               []() { funC::asm_preamble = true; });
  p.add_checked_option('O', "opt-level", "Set optimization level (2 by default)", [&](td::Slice arg) -> td::Status {
    TRY_RESULT_ASSIGN(funC::opt_level, td::to_integer_safe<int>(arg));
    funC::opt_level = std::max(0, funC::opt_level);
    return td::Status::OK();
  });
  p.add_option('P', "program-envelope", "Envelope code into PROGRAM{ ... }END>c",
               []() { funC::program_envelope = true; });
  p.add_option('S', "stack-layout-comments", "Include stack layout comments in the output code",
  ...
  p.add_option('W', "boc-output",
               "Include Fift code to serialize and save generated code into specified BoC file. Enables -A and -P.",
               [](td::Slice arg) {
  ...
  p.add_option('V', "version", "Show FunC build information", []() {
  ...
  if (S.is_error()) {
    std::cerr << "failed to parse options: " << S.error().message().c_str() << "\n";
    return 2;
  }
