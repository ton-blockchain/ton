...
void run_check_external_message(Ref<ExtMessage> message, td::actor::ActorId<ValidatorManager> manager, Mevton* mevton,
                                td::Promise<td::Ref<ExtMessage>> promise) {
  ExtMessageQ::run_message(std::move(message), std::move(manager), mevton, std::move(promise));
}
