...
void ValidatorGroup::create_session() {
  ...
  double catchain_delay = opts_->get_catchain_max_block_delay() ? opts_->get_catchain_max_block_delay().value() : 0.5;
  double catchain_delay_slow =
      std::max(catchain_delay,
               opts_->get_catchain_max_block_delay_slow() ? opts_->get_catchain_max_block_delay_slow().value() : 0.0);
