...
td::Status check_delegation(const Delegation& delegation, const PeerValidator& leader, td::uint32 slot,
  ...
  auto registered = bus.collators_by_validator.find(leader.short_id);
  if (registered == bus.collators_by_validator.end() ||
      std::find(registered->second.begin(), registered->second.end(), collator_id) == registered->second.end()) {
    return td::Status::Error("Candidate is signed by a collator not registered by the leader");
  }

  td::uint32 window_start = slot - slot % bus.config.slots_per_leader_window;
  auto window_payload = create_serialize_tl_object<tl::collationWindow>(window_start, collator_id.bits256_value());
  if (!leader.check_signature(bus.session_id, window_payload, delegation.signature)) {
