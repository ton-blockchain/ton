...
class CollatorProducerImpl : public td::actor::SpawnsWith<Bus>, public td::actor::ConnectsTo<Bus> {
 public:
  ...
  void handle(BusHandle, std::shared_ptr<const IncomingProtocolMessage> message) {
    ...
    auto window_payload =
        create_serialize_tl_object<consensus::tl::collationWindow>(window_start, bus.local_adnl_id.bits256_value());
    if (!leader.check_signature(bus.session_id, window_payload, request->signature_)) {
