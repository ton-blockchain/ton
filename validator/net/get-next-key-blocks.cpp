...
GetNextKeyBlocks::GetNextKeyBlocks(BlockIdExt block_id, td::uint32 limit, QuerySender query_sender, td::uint32 priority,
...
void GetNextKeyBlocks::abort_query(td::Status reason) {
  if (promise_) {
    if (reason.code() == ErrorCode::notready || reason.code() == ErrorCode::timeout) {
      VLOG(full_node, DEBUG) << "failed to download next key blocks after " << block_id_ << " from "
                             << query_sender_->to_str() << ": " << reason;
    } else {
      VLOG(full_node, INFO) << "failed to download next key blocks after " << block_id_ << " from "
                            << query_sender_->to_str() << ": " << reason;
    }
    if (res_.size() > 0) {
      VLOG(full_node, DEBUG) << "downloaded " << res_.size() << " next key blocks after " << block_id_ << " from "
...
void GetNextKeyBlocks::got_download_token(std::unique_ptr<ActionToken> token) {
  ...
  VLOG(full_node, DEBUG) << "downloading next key blocks after " << block_id_ << " from " << query_sender_->to_str();
  ...
  query_sender_->send_query(
      ...
      td::Timestamp::in(1.0), 1024, std::move(P));
}
...
void GetNextKeyBlocks::download_next_proof() {
  ...
  td::actor::create_actor<DownloadProof>("downloadproofreq", block_id, false, true, query_sender_, priority_, timeout_,
