...
void QuicSender::send_message(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst, td::BufferSlice data){
  find_out_connection({src, dst}, [data = std::move(data), src, dst] (td::Result<OutboundConnection*> R) mutable {
    if (R.is_error()) {
      LOG(ERROR) << R.move_as_error_prefix(PSTRING() << "dropping message " << src << '>' << dst << " because connection failed");
      return;
    }
    auto client = R.move_as_ok()->client.get();
    td::actor::send_closure(client, &QuicClient::open_stream, [data = std::move(data), client, src, dst] (td::Result<QuicStreamID> res) mutable {
      if (res.is_error()) {
        LOG(ERROR) << res.move_as_error_prefix(PSTRING() << "dropping message " << src << '>' << dst << " because stream failed");
        return;
      }
...
void QuicSender::send_query(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst, std::string name, td::Promise<td::BufferSlice> promise, td::Timestamp timeout, td::BufferSlice data){
  find_out_connection({src, dst}, [data = std::move(data), promise = std::move(promise), src, dst] (td::Result<OutboundConnection*> R) mutable {
    if (R.is_error()) {
      LOG(ERROR) << R.move_as_error_prefix(PSTRING() << "dropping query " << src << '>' << dst << " because connection failed");
      return;
    }
    auto conn = R.move_as_ok();
    auto client = conn->client.get();
    td::actor::send_closure(client, &QuicClient::open_stream, [data = std::move(data), promise = std::move(promise), conn, client, src, dst] (td::Result<QuicStreamID> res) mutable {
      if (res.is_error()) {
        LOG(ERROR) << res.move_as_error_prefix(PSTRING() << "dropping query " << src << '>' << dst << " because stream failed");
        return;
      }
      auto sid = res.move_as_ok();
...
void QuicSender::send_query_ex(adnl::AdnlNodeIdShort src, adnl::AdnlNodeIdShort dst, std::string name, td::Promise<td::BufferSlice> promise, td::Timestamp timeout, td::BufferSlice data, td::uint64 max_answer_size){
  send_query(src, dst, name, std::move(promise), timeout, std::move(data));
}

void QuicSender::get_conn_ip_str(adnl::AdnlNodeIdShort l_id, adnl::AdnlNodeIdShort p_id, td::Promise<td::string> promise){
...
td::actor::Task<> QuicSender::add_local_id(adnl::AdnlNodeIdShort local_id){
  ...
  public:
    InConnectionCallback(adnl::AdnlNodeIdShort local_id, td::actor::ActorId<QuicSender> sender) : local_id_(local_id), sender_(sender) {}
    ...
    void on_stream_end(const td::IPAddress& peer, QuicStreamID sid) override {
      ...
      ton_api::downcast_call(*response, td::overloaded(
        ...
        [this, &conn] (ton_api::quic_message& message) {
          td::actor::send_closure(sender_, &QuicSender::after_in_message, AdnlPath{conn.peer_id_, local_id_}, std::move(message.data_));
        }
      ));
    }
  ...
  auto res = QuicServer::listen(port, cert_file_, key_file_, std::make_unique<InConnectionCallback>(local_id, actor_id(this)));
  if (res.is_error())
...
void QuicSender::find_out_connection(AdnlPath path, td::Promise<OutboundConnection*> P){
  ...
  if (iter == outbound_.end())
    ...
    if (iter->second.ready_received)
      ...
      iter->second.ready = [P = std::move(P), P1 = std::move(iter->second.ready)] (td::Result<OutboundConnection*> R) mutable {
...
void QuicSender::create_connection(AdnlPath path, td::Promise<OutboundConnection*> P){
  td::actor::send_closure(adnl_, &adnl::Adnl::get_peer_node, path.first, path.second, [this, path, P = std::move(P)] (td::Result<adnl::AdnlNode> res) mutable {
    ...
    public:
      OutConnectionCallback(AdnlPath path, td::actor::ActorId<QuicSender> sender) : sender_(std::move(sender)), path_(std::move(path)) {}
      ...
      void on_stream_end(QuicStreamID sid) override {
        ...
        ton_api::downcast_call(*response, td::overloaded(
          ...
          [this, sid] (ton_api::quic_answer& answer) {
            td::actor::send_closure(sender_, &QuicSender::after_out_query_answer, path_, sid, std::move(answer.data_));
          }
        ));
      }
...
void QuicSender::after_out_connection_created(AdnlPath path){
  ...
  td::actor::send_closure(client, &QuicClient::open_stream, [path, client] (td::Result<QuicStreamID> res) mutable {
    if (res.is_error()) {
      LOG(ERROR) << res.move_as_error_prefix(PSTRING() << "failed to open stream for connection " << path.first << '>' << path.second);
      return;
    }
    auto sid = res.move_as_ok();
...
void QuicSender::after_in_query_answer(adnl::AdnlNodeIdShort local_id, td::IPAddress peer, QuicStreamID sid, td::BufferSlice data){
