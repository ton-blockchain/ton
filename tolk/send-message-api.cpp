...
static void append_bitwise_and_shard_mask(CodeBlob& code, AnyV origin, var_idx_t ir_addr_hash, var_idx_t ir_shard_depth) {
  ...
  code.add_call(origin, ir_mask, {code.create_int(origin, 256, ""), ir_shard_depth}, lookup_function("_-_"));
  code.add_call(origin, ir_mask, {ir_one, ir_mask[0]}, lookup_function("_<<_"));
  code.add_call(origin, ir_mask, {ir_mask[0], ir_one}, lookup_function("_-_"));
  code.add_call(origin, {ir_addr_hash}, {ir_addr_hash, ir_mask[0]}, lookup_function("_&_"));
}
...
std::vector<var_idx_t> generate_createMessage(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& ir_options) {
  ...
  if (body_store_as_ref && !body_already_ref) {
    ...
    code.add_call(origin, ir_ref_builder, {}, f_beginCell);
    ...
    code.add_call(origin, ir_ref_cell, std::move(ir_ref_builder), f_endCell);
    ir_body = std::move(ir_ref_cell);
  }

  std::vector ir_builder = code.create_var(TypeDataSlice::create(), origin, "b");
  code.add_call(origin, ir_builder, {}, f_beginCell);
  ...
  Op& if_oldBounceFormat = code.add_if_else(origin, ir_bounce_is_bool);
  {
  ...
  }
  {
    ...
    code.add_call(origin, ir_not_NoBounce, {ir_bounce[0], ir_zero}, lookup_function("_!=_"));
  ...
  Op& if_address = code.add_if_else(origin, ir_dest_is_address);
  {
  ...
  }
  {
    ...
    Op& if_AutoDeploy = code.add_if_else(origin, ir_dest_is_AutoDeploy);
    {
      ...
      Op& if_ContractState = code.add_if_else(origin, ir_dest_ad.is_ContractState(code, origin));
      {
        // input is `dest: { ... stateInit: { code, data } }`
        code.push_set_cur(if_ContractState.block0);
        Op& if_sharded = code.add_if_else(origin, ir_dest_ad.is_AddressSharding(code, origin));
        {
          ...
          code.add_call(origin, ir_hash, std::move(args), lookup_function("StateInit.calcHashPrefixCodeData"));
          code.close_pop_cur(origin);
        }
        {
          ...
          code.add_call(origin, ir_hash, std::move(args), lookup_function("StateInit.calcHashCodeData"));
          code.close_pop_cur(origin);
        }
        code.close_pop_cur(origin);
      }
      {
        ...
        code.add_call(origin, ir_hash, std::move(args), lookup_function("cell.hash"));
        code.close_pop_cur(origin);
      }
      Op& if_sharded = code.add_if_else(origin, ir_dest_ad.is_AddressSharding(code, origin));
      {
        ...
        code.add_call(origin, ir_lowerD, {code.create_int(origin, 256, ""), ir_dest_ad.ir_shardDepth}, lookup_function("_-_"));
        ...
        code.add_call(origin, ir_shardPrefix, std::move(args_subslice), lookup_function("slice.getMiddleBits"));
    ...
    }
    {
      ...
      Op& if_builder = code.add_if_else(origin, ir_dest_is_builder);
      {
  ...
  Op& if_coins = code.add_if_else(origin, ir_is_coins);
  {
  ...
  }
  {
    ...
    code.add_call(origin, ir_eq_RichBounce, {ir_bounce[0], code.create_int(origin, 2, "(enum-RichBounce)")}, lookup_function("_==_"));
    ...
    }
    {
      ...
      code.add_call(origin, ir_eq_RichBounceRoot, {ir_bounce[0], code.create_int(origin, 3, "(enum-RichBounceRoot)")}, lookup_function("_==_"));
  ...
  Op& if_no_init = code.add_if_else(origin, ir_dest_is_AutoDeploy);
  {
  ...
  code.add_call(origin, ir_cell, std::move(ir_builder), f_endCell);
  return ir_cell;
}
...
std::vector<var_idx_t> generate_createExternalLogMessage(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& args) {
  ...
  }
  {
    ...
    }
    {
      ...
      Op& if_topic_uint = code.add_if_else(origin, ir_if_topic_uint);
      {
...
std::vector<var_idx_t> generate_address_buildInAnotherShard(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& args) {
  ...
  code.add_call(origin, ir_offsetB, {ir_offsetB[0], ir_shard_options[0]}, lookup_function("_+_"));
  std::vector ir_headB = code.create_tmp_var(TypeDataSlice::create(), origin, "(headB)");
  code.add_call(origin, ir_headB, {ir_shard_options[1], ir_offsetB[0]}, lookup_function("slice.getFirstBits"));

  std::vector ir_builder = code.create_var(TypeDataBuilder::create(), origin, "b");
  code.add_call(origin, ir_builder, {}, lookup_function("beginCell"));
  code.add_call(origin, ir_builder, {ir_builder[0], ir_headB[0]}, lookup_function("builder.storeSlice"));

  std::vector ir_restLenA = {code.create_int(origin, 256, "(last-addrA)")};
  code.add_call(origin, ir_restLenA, {ir_restLenA[0], ir_shard_options[0]}, lookup_function("_-_"));
  std::vector ir_tailA = code.create_tmp_var(TypeDataSlice::create(), origin, "(tailA)");
  code.add_call(origin, ir_tailA, {args[0][0], ir_restLenA[0]}, lookup_function("slice.getLastBits"));
  code.add_call(origin, ir_builder, {ir_builder[0], ir_tailA[0]}, lookup_function("builder.storeSlice"));
...
std::vector<var_idx_t> generate_address_calculateInAnotherShard(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& args) {
  ...
  code.add_call(origin, ir_builder, ir_builder, lookup_function("builder.toSlice"));
  return ir_builder;
}
...
std::vector<var_idx_t> generate_AutoDeployAddress_buildAddress(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& ir_options) {
  ...
  std::vector ir_hash = code.create_tmp_var(TypeDataInt::create(), origin, "(addr-hash)");
  Op& if_ContractState = code.add_if_else(origin, ir_self.is_ContractState(code, origin));
  {
    // called `{ ... stateInit: { code, data } }`
    code.push_set_cur(if_ContractState.block0);
    Op& if_sharded = code.add_if_else(origin, ir_self.is_AddressSharding(code, origin));
    {
  ...
  Op& if_sharded = code.add_if_else(origin, ir_self.is_AddressSharding(code, origin));
  {
    ...
    std::vector ir_lowerD = code.create_tmp_var(TypeDataInt::create(), origin, "(lowerD)");
    code.add_call(origin, ir_lowerD, {code.create_int(origin, 256, ""), ir_self.ir_shardDepth}, lookup_function("_-_"));
...
std::vector<var_idx_t> generate_AutoDeployAddress_addressMatches(FunctionPtr called_f, CodeBlob& code, AnyV origin, const std::vector<std::vector<var_idx_t>>& ir_self_and_addr) {
  ...
  Op& if_sharded1 = code.add_if_else(origin, ir_self.is_AddressSharding(code, origin));
  {
  ...
  code.add_call(origin, ir_addr_wc_hash, ir_self_and_addr[1], lookup_function("address.getWorkchainAndHash"));

  // now calculate `hash &= mask` (the same as we did earlier for stateInitHash)
  Op& if_sharded2 = code.add_if_else(origin, ir_self.is_AddressSharding(code, origin));
  {
  ...
  code.add_call(origin, ir_eq_hash, {ir_addr_wc_hash[1], ir_hash[0]}, lookup_function("_==_"));
  std::vector ir_eq_wc = code.create_tmp_var(TypeDataInt::create(), origin, "(eq-wc)");
  code.add_call(origin, ir_eq_wc, {ir_addr_wc_hash[0], ir_self.workchain}, lookup_function("_==_"));

  std::vector ir_bool_result = code.create_tmp_var(TypeDataBool::create(), origin, "(is-addr-result)");
  code.add_call(origin, ir_bool_result, {ir_eq_hash[0], ir_eq_wc[0]}, lookup_function("_&_"));
