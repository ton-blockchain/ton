...
static void fire(const FunctionData* cur_f, SrcLocation loc, const std::string& message) {
  throw ParseError(cur_f, loc, message);
}

// fire an error on `!cell` / `+slice`
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD
static void fire_error_cannot_apply_operator(const FunctionData* cur_f, SrcLocation loc, std::string_view operator_name, AnyExprV unary_expr) {
...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD
static void fire_error_cannot_apply_operator(const FunctionData* cur_f, SrcLocation loc, std::string_view operator_name, AnyExprV lhs, AnyExprV rhs) {
  std::string op = static_cast<std::string>(operator_name);
...
static void check_function_argument_passed(const FunctionData* cur_f, TypePtr param_type, AnyExprV ith_arg, bool is_obj_of_dot_call) {
...
static void check_function_argument_mutate_back(const FunctionData* cur_f, TypePtr param_type, AnyExprV ith_arg, bool is_obj_of_dot_call) {
...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD
static void fire_error_assign_always_null_to_variable(const FunctionData* cur_f, SrcLocation loc, const LocalVarData* assigned_var, bool is_assigned_null_literal) {
...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD
static void fire_error_cannot_put_non1_stack_width_arg_to_tuple(const FunctionData* cur_f, SrcLocation loc, TypePtr inferred_type) {
...
static void handle_possible_compiler_internal_call(const FunctionData* cur_f, V<ast_function_call> v) {
...
class CheckInferredTypesVisitor final : public ASTVisitorFunctionBody {
  const FunctionData* cur_f = nullptr;     // may be nullptr if checking `const a = ...` init_value
...
protected:
  ...
  void visit(V<ast_binary_operator> v) override {
    ...
    switch (v->tok) {
      ...
      // for nullable, it also works: (int == int?) is ok, (bool? == bool?) is ok
      case tok_eq:
      case tok_neq: {
        TypePtr lhs_type = unwrap_nullable(lhs->inferred_type);
        TypePtr rhs_type = unwrap_nullable(rhs->inferred_type);
        bool both_int = expect_integer(lhs_type) && expect_integer(rhs_type);
        bool both_bool = expect_boolean(lhs_type) && expect_boolean(rhs_type);
  ...
  void visit(V<ast_is_null_check> v) override {
    ...
    bool is_nullable = expr_type->can_rhs_be_assigned(TypeDataNullLiteral::create());
    if (!is_nullable) {
      v->loc.show_warning("type " + to_string(expr_type) + " is always not null");
    }
  }
    ...
    if (auto lhs_var = lhs->try_as<ast_local_var_lhs>()) {
      ...
      if (declared_type) {
        ...
        if (rhs_type == TypeDataNullLiteral::create()) {
          fire_error_assign_always_null_to_variable(cur_f, err_loc->loc, lhs_var->var_ref->try_as<LocalVarData>(), corresponding_maybe_rhs && corresponding_maybe_rhs->type == ast_null_keyword);
        }
      }
      return;
    }
 ...
 public:
  ...
  void start_visiting_function(const FunctionData* fun_ref, V<ast_function_declaration> v_function) override {
    cur_f = fun_ref;
