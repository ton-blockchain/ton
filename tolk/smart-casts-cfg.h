...
struct SinkExpression {
  const LocalVarData* const var_ref;    // smart casts and data flow applies only to locals
  const uint64_t index_path;            // 0 for just `v`; for `v.N` it's (N+1), for `v.N.M` it's (N+1) + (M+1)<<8, etc.

  SinkExpression()
    : var_ref(nullptr), index_path(0) {}
  explicit SinkExpression(const LocalVarData* var_ref)
    : var_ref(var_ref), index_path(0) {}
  explicit SinkExpression(const LocalVarData* var_ref, uint64_t index_path)
...
class FlowContext {
  ...
  void invalidate_all_subfields(const LocalVarData* var_ref, uint64_t parent_path, uint64_t parent_mask);
