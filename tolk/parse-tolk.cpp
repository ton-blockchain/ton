...
TypeExpr* parse_type1(Lexer& lex) {
  switch (lex.tp()) {
    ...
    case _Ident: {
      auto sym = lookup_symbol(lex.cur().val);
...
FormalArg parse_formal_arg(Lexer& lex, int fa_idx) {
  ...
  loc = lex.cur().loc;
  if (prohibited_var_names.count(symbols.get_name(lex.cur().val))) {
    throw ParseError{
        loc, PSTRING() << "symbol `" << symbols.get_name(lex.cur().val) << "` cannot be redefined as a variable"};
  }
...
void parse_global_var_decl(Lexer& lex) {
  ...
  loc = lex.cur().loc;
  SymDef* sym_def = define_global_symbol(lex.cur().val, false, loc);
  if (!sym_def) {
...
SymValAsmFunc* parse_asm_func_body(Lexer& lex, TypeExpr* func_type, const FormalArgList& arg_list, TypeExpr* ret_type,
  ...
  if (asm_ops.empty()) {
    throw ParseError{lex.cur().loc, "string with assembler instruction expected"};
  }
  lex.expect(';');
  std::string crc_s;
...
std::vector<TypeExpr*> parse_type_var_list(Lexer& lex) {
  ...
  while (true) {
    ...
    if (lex.tp() != _Ident) {
      throw ParseError{lex.cur().loc, "free type identifier expected"};
    }
    auto loc = lex.cur().loc;
    if (prohibited_var_names.count(symbols.get_name(lex.cur().val))) {
      throw ParseError{loc, PSTRING() << "symbol `" << symbols.get_name(lex.cur().val)
...
std::string tolk_ver_test = tolk_version;
...
void parse_pragma(Lexer& lex) {
  ...
  if (!pragma_name.compare("version") || !pragma_name.compare("not-version")) {
    ...
    std::istringstream iss(tolk_ver_test);
    std::string s;
    ...
    if ((match && negate) || (!match && !negate)) {
      pragma.error(std::string("Tolk version ") + tolk_ver_test + " does not satisfy condition " + semver_expr);
    }
  } else if (!pragma_name.compare("test-version-set")) {
    if (lex.tp() != _String) {
      lex.cur().error("version string expected");
    }
    tolk_ver_test = lex.cur().str;
    lex.next();
...
std::vector<const FileDescr*> source_fdescr;

std::map<std::string, FileDescr*> source_files;
std::stack<SrcLocation> inclusion_locations;

void parse_include(Lexer& lex, const FileDescr* fdescr) {
...
bool parse_source_file(const char* filename, Lexem lex, bool is_main) {
...
bool parse_source_stdin() {
  FileDescr* cur_source = new FileDescr{"stdin", true};
