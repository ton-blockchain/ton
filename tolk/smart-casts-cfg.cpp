...
void FlowContext::invalidate_all_subfields(const LocalVarData* var_ref, uint64_t parent_path, uint64_t parent_mask) {
...
// mark control flow reachable; this method shouldn't exist at all, but currently does, see comments at usages
void FlowContext::mark_reachable() {
  unreachable = false;
}
...
SinkExpression extract_sink_expression_from_vertex(AnyExprV v) {
  if (auto as_ref = v->try_as<ast_reference>()) {
    if (const auto* var_ref = as_ref->sym->try_as<LocalVarData>()) {
