...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD
static void fire_error_calling_asm_function_with_non1_stack_width_arg(const FunctionData* cur_f, SrcLocation loc, const FunctionData* fun_ref, const std::vector<TypePtr>& substitutions, int arg_idx) {
...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD
static void fire_error_cannot_deduce_untyped_tuple_access(const FunctionData* cur_f, SrcLocation loc, int index) {
...
class InferTypesAndCallsAndFieldsVisitor final {
  const FunctionData* cur_f = nullptr;
  ...
  ExprFlow infer_ternary_operator(V<ast_ternary_operator> v, FlowContext&& flow, bool used_as_condition, TypePtr hint) {
    ...
    if (after_cond.false_flow.is_unreachable()) {
      return after_true;
    }
    if (after_cond.true_flow.is_unreachable()) {
      return after_false;
    }
  ...
  ExprFlow infer_is_null_check(V<ast_is_null_check> v, FlowContext&& flow, bool used_as_condition) {
    ...
    if (SinkExpression s_expr = extract_sink_expression_from_vertex(v->get_expr())) {
      ...
      } else {                                                 // `expr == null` when true null, when false !null
        true_flow.register_known_type(s_expr, TypeDataNullLiteral::create());
        false_flow.register_known_type(s_expr, non_null_type);
      }
      if (v->is_negated) {    // `expr != null` check, actually
        std::swap(true_flow, false_flow);
      }
    }
  ...
  std::vector<TypePtr> collect_fun_generic_substitutions_from_manually_specified(SrcLocation loc, const FunctionData* fun_ref, V<ast_instantiationT_list> instantiationT_list) const {
  ...
  const FunctionData* check_and_instantiate_generic_function(SrcLocation loc, const FunctionData* fun_ref, std::vector<TypePtr>&& substitutionTs) const {
  ...
  FlowContext process_while_statement(V<ast_while_statement> v, FlowContext&& flow) {
    ...
    if (after_cond2.false_flow.is_unreachable()) {
      // example: `while (true) { if (smth) return; }`
      // currently, IR level can't detect endless loops, and if no implicit return inserted, it fails
      // the hack is to treat after-while reachable at cfg level
      after_cond2.false_flow.mark_reachable();
    }
  ...
  FlowContext process_do_while_statement(V<ast_do_while_statement> v, FlowContext&& flow) {
    ...
    if (after_cond2.false_flow.is_unreachable()) {
      after_cond2.false_flow.mark_reachable();   // same hack as for `while`, read above
    }
    return std::move(after_cond2.false_flow);
  }
