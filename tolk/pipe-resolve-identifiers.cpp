...
static void fire_error_undefined_symbol(const FunctionData* cur_f, V<ast_identifier> v) {
  if (v->name == "self") {
...
GNU_ATTRIBUTE_NORETURN GNU_ATTRIBUTE_COLD
static void fire_error_unknown_type_name(const FunctionData* cur_f, SrcLocation loc, const std::string &text) {
  throw ParseError(cur_f, loc, "unknown type name `" + text + "`");
}

static void check_import_exists_when_using_sym(const FunctionData* cur_f, AnyV v_usage, const Symbol* used_sym) {
...
struct TypeDataResolver {
  ...
  static TypePtr resolve_identifiers_in_type_data(const FunctionData* cur_f, TypePtr type_data, const GenericsDeclaration* genericTs) {
