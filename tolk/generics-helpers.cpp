...
GenericSubstitutionsDeduceForCall::GenericSubstitutionsDeduceForCall(const FunctionData* fun_ref)
  : fun_ref(fun_ref) {
...
TypePtr GenericSubstitutionsDeduceForCall::auto_deduce_from_argument(const FunctionData* cur_f, SrcLocation loc, TypePtr param_type, TypePtr arg_type) {
  try {
    ...
    throw ParseError(cur_f, loc, ex.message + " for generic function `" + fun_ref->as_human_readable() + "`");
  }
}
