  ...
  void visit(V<ast_reference> v) override {
    if (extract_sink_expression_from_vertex(v) == s_expr) {
      ...
      if (!is_subj_of_dot) {
        ...
        if (v->is_lvalue && !v->is_rvalue) {
          err("can not assign to a lazy variable `{}`\n""hint: `lazyVar = rhs` is forbidden, use `lazyVar.field = rhs`", s_expr.var_ref).fire(v, cur_f);
        }
      }
    ...
    if (fun_ref && fun_ref->does_accept_self() && v->dot_obj_is_self) {
      ...
      if (extract_sink_expression_from_vertex(dot_obj) == s_expr) {
        ...
        if (fun_ref->is_builtin() && fun_ref->is_instantiation_of_generic_function() && fun_ref->base_fun_ref->name == "T.toCell") {
          ...
          if (dot_obj->kind != ast_reference) {    // not just `lazyVar.toCell()`
            parent::visit(v->get_callee());     // but `(lazyVar.field = rhs).toCell()` or so
          }
