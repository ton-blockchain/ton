...
static std::vector<var_idx_t> pre_compile_let(CodeBlob& code, AnyExprV lhs, AnyExprV rhs, SrcLocation loc) {
  ...
  if (lhs->type == ast_typed_tuple && rhs->type == ast_typed_tuple) {
    ...
    std::vector<var_idx_t> rvect = code.create_tmp_var(TypeDataTuple::create(), loc, "(tuple)");
    code.emplace_back(lhs->loc, Op::_Tuple, rvect, std::move(right));
    return rvect;
  }
...
// So, this null flag should be implicitly added (zero value, since a variable is not null).
...
static std::vector<var_idx_t> process_ternary_operator(V<ast_ternary_operator> v, CodeBlob& code, TypePtr target_type) {
  ...
  Op& if_op = code.emplace_back(v->loc, Op::_If, cond);
  code.push_set_cur(if_op.block0);
  code.emplace_back(v->get_when_true()->loc, Op::_Let, rvect, pre_compile_expr(v->get_when_true(), code, target_type));
  code.close_pop_cur(v->get_when_true()->loc);
  code.push_set_cur(if_op.block1);
  code.emplace_back(v->get_when_false()->loc, Op::_Let, rvect, pre_compile_expr(v->get_when_false(), code, target_type));
  code.close_pop_cur(v->get_when_false()->loc);
  return rvect;   // no transition to target type, since every branch already transitions
}
...
static std::vector<var_idx_t> process_is_null_check(V<ast_is_null_check> v, CodeBlob& code, TypePtr target_type) {
  ...
  if (expr_type->try_as<TypeDataNullable>()) {
    bool has_separate_null_flag = expr_ir_idx.size() > 1;
    if (has_separate_null_flag) {
      ...
      const FunctionData* eq_sym = lookup_global_symbol("_==_")->try_as<FunctionData>();
      code.emplace_back(v->loc, Op::_Call, isnull_ir_idx, std::vector{expr_ir_idx.back(), zero_ir_idx[0]}, eq_sym);
    } else {
      const FunctionData* builtin_sym = lookup_global_symbol("__isNull")->try_as<FunctionData>();
  ...
  if (v->is_negated) {
    const FunctionData* not_sym = lookup_global_symbol("!b_")->try_as<FunctionData>();
