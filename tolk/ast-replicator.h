...
class ASTReplicator final {
  ...
  static V<ast_type_leaf_text> clone(V<ast_type_leaf_text> v) {
    return createV<ast_type_leaf_text>(v->range, v->text);
  }
  static V<ast_type_question_nullable> clone(V<ast_type_question_nullable> v) {
    return createV<ast_type_question_nullable>(v->range, clone(v->get_inner()));
  }
  ...
  static V<ast_type_vertical_bar_union> clone(V<ast_type_vertical_bar_union> v) {
    return createV<ast_type_vertical_bar_union>(v->range, clone(v->get_variants()));
  }
  static V<ast_type_triangle_args> clone(V<ast_type_triangle_args> v) {
    return createV<ast_type_triangle_args>(v->range, clone(v->get_inner_and_args()));
  }
  ...
  static V<ast_int_const> clone(V<ast_int_const> v) {
    return createV<ast_int_const>(v->range, v->intval, v->orig_str);
  }
  static V<ast_string_const> clone(V<ast_string_const> v) {
    return createV<ast_string_const>(v->range, v->str_val);
  }
  static V<ast_bool_const> clone(V<ast_bool_const> v) {
    return createV<ast_bool_const>(v->range, v->bool_val);
  }
  static V<ast_null_keyword> clone(V<ast_null_keyword> v) {
    return createV<ast_null_keyword>(v->range);
  }
  static V<ast_argument> clone(V<ast_argument> v) {
    return createV<ast_argument>(v->range, clone(v->get_expr()), v->passed_as_mutate);
  }
  static V<ast_argument_list> clone(V<ast_argument_list> v) {
    return createV<ast_argument_list>(v->range, clone(v->get_arguments()));
  }
  static V<ast_dot_access> clone(V<ast_dot_access> v) {
    return createV<ast_dot_access>(v->range, clone(v->get_obj()), clone(v->get_identifier()), v->has_instantiationTs() ? clone(v->get_instantiationTs()) : nullptr);
  }
  static V<ast_function_call> clone(V<ast_function_call> v) {
    return createV<ast_function_call>(v->range, clone(v->get_callee()), clone(v->get_arg_list()));
  }
  static V<ast_underscore> clone(V<ast_underscore> v) {
    return createV<ast_underscore>(v->range);
  }
  static V<ast_assign> clone(V<ast_assign> v) {
    return createV<ast_assign>(v->range, clone(v->get_lhs()), clone(v->get_rhs()));
  }
  static V<ast_set_assign> clone(V<ast_set_assign> v) {
    return createV<ast_set_assign>(v->range, v->operator_range, v->operator_name, v->tok, clone(v->get_lhs()), clone(v->get_rhs()));
  }
  static V<ast_unary_operator> clone(V<ast_unary_operator> v) {
    return createV<ast_unary_operator>(v->range, v->operator_range, v->operator_name, v->tok, clone(v->get_rhs()));
  }
  static V<ast_binary_operator> clone(V<ast_binary_operator> v) {
    return createV<ast_binary_operator>(v->range, v->operator_range, v->operator_name, v->tok, clone(v->get_lhs()), clone(v->get_rhs()));
  }
  ...
  static V<ast_is_type_operator> clone(V<ast_is_type_operator> v) {
    return createV<ast_is_type_operator>(v->range, clone(v->get_expr()), clone(v->type_node), v->is_negated);
  }
  static V<ast_not_null_operator> clone(V<ast_not_null_operator> v) {
    return createV<ast_not_null_operator>(v->range, clone(v->get_expr()));
  }
  static V<ast_lazy_operator> clone(V<ast_lazy_operator> v) {
    return createV<ast_lazy_operator>(v->range, clone(v->get_expr()));
  }
  static V<ast_match_expression> clone(V<ast_match_expression> v) {
    return createV<ast_match_expression>(v->range, clone(v->get_all_children()));
  }
  ...
  static V<ast_object_body> clone(V<ast_object_body> v) {
    return createV<ast_object_body>(v->range, clone(v->get_all_fields()));
  }
  ...
  static V<ast_empty_statement> clone(V<ast_empty_statement> v) {
    return createV<ast_empty_statement>(v->range);
  }
  static V<ast_block_statement> clone(V<ast_block_statement> v) {
    return createV<ast_block_statement>(v->range, clone(v->get_items()));
  }
  static V<ast_return_statement> clone(V<ast_return_statement> v) {
    return createV<ast_return_statement>(v->range, clone(v->get_return_value()));
  }
  static V<ast_if_statement> clone(V<ast_if_statement> v) {
    return createV<ast_if_statement>(v->range, v->is_ifnot, clone(v->get_cond()), clone(v->get_if_body()), clone(v->get_else_body()));
  }
  static V<ast_repeat_statement> clone(V<ast_repeat_statement> v) {
    return createV<ast_repeat_statement>(v->range, clone(v->get_cond()), clone(v->get_body()));
  }
  static V<ast_while_statement> clone(V<ast_while_statement> v) {
    return createV<ast_while_statement>(v->range, clone(v->get_cond()), clone(v->get_body()));
  }
  ...
  static V<ast_assert_statement> clone(V<ast_assert_statement> v) {
    return createV<ast_assert_statement>(v->range, clone(v->get_cond()), clone(v->get_thrown_code()));
  }
  ...
  static V<ast_identifier> clone(V<ast_identifier> v) {
    return createV<ast_identifier>(v->range, v->name);
  }
  static V<ast_genericsT_item> clone(V<ast_genericsT_item> v) {
    return createV<ast_genericsT_item>(v->range, v->nameT, clone(v->default_type_node));
  }
  static V<ast_genericsT_list> clone(V<ast_genericsT_list> v) {
    return createV<ast_genericsT_list>(v->range, clone(v->get_items()));
  }
  static V<ast_instantiationT_item> clone(V<ast_instantiationT_item> v) {
    return createV<ast_instantiationT_item>(v->range, clone(v->type_node));
  }
