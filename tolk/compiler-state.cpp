...
bool CompilerSettings::parse_path_mapping_cmd_arg(const std::string& cmd_arg) {
  ...
  if (!cmd_arg.starts_with('@') || pos == std::string::npos || pos == 1 || pos >= cmd_arg.size()) {
  ...
  if (!get_path_mapping(at_prefix).empty()) {
    std::cerr << "duplicated path mapping " << at_prefix << std::endl;
    return false;
  }
