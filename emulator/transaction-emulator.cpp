...
namespace emulator {
td::Result<> TransactionEmulator::prepare_emulation(block::Account& account, ton::UnixTime& utime,
                                                    ton::LogicalTime& lt) {
...
td::Result<TransactionEmulator::EmulationSuccess> TransactionEmulator::emulate_transaction(
    block::Account&& account, td::Ref<vm::Cell> original_trans) {
  ...
  switch (tag) {
    ...
    case block::gen::TransactionDescr::trans_tick_tock: {
      ...
      trans_type =
          tick_tock.is_tock ? block::transaction::Transaction::tr_tock : block::transaction::Transaction::tr_tick;
      break;
    }
  ...
  if (auto emulation_result_ptr = dynamic_cast<EmulationSuccess*>(emulation.get())) {
    ...
    return td::Status::Error(PSTRING() << "VM Log: " << emulation_not_accepted_ptr->vm_log
...
td::Result<TransactionEmulator::EmulationChain> TransactionEmulator::emulate_transactions_chain(
    block::Account&& account, std::vector<td::Ref<vm::Cell>>&& original_transactions) {
...
bool TransactionEmulator::check_state_update(const block::Account& account,
                                             const block::gen::Transaction::Record& trans) {
