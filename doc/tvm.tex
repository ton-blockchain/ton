...
\nxsubpoint\label{sp:cell.depth}\emb{The depth of a cell}
The same way, every cell $c$ has another attribute $\Depth(c)$. The depth of a cell is always equal to the maximum of the levels of all its children $c_i$ plus one:
\begin{equation}
  \Depth(c)=\max_{1\leq i\leq r}\Depth(c_i)\quad+1.
\end{equation}
If $r=0$, $\Depth(c)=0$. 
...
\item Then for each of the $r$ cell references 2 bytes encoding $\Depth(c_i)$ explained above in~\ptref{sp:cell.depth}.
...
\end{enumerate}
In this way, $2+\lceil b/8\rceil+(2+32)r$ bytes of $\CellRepr(c)$ are obtained.
