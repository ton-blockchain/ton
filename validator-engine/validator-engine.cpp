...
void ValidatorEngine::add_dht(ton::PublicKeyHash id) {
  if (dht_client_ || !config_.validators.empty()) {
    dht_nodes_[id] =
...
#endif
  p.add_option('\0', "dht-client", "run DHT in client mode",
               [&]() { acts.push_back([&] { td::actor::send_closure(x, &ValidatorEngine::set_dht_client, true); }); });
