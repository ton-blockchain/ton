...
### 4. Сборка GraphLogger (основная проверка simulation/)

```bash
ninja simulation_graph_logger -j4 2>&1 | tail -30
```

Ожидаемый результат: `[N/N] Linking CXX static library ...simulation_graph_logger.a` без ошибок.
...
| Проверка | Ожидание |
|---|---|
| `ninja simulation_graph_logger` | exit 0, `.a` создан без ошибок |
...
2. Если `ninja simulation_graph_logger` падает — сохранить первые 50 строк ошибки (`2>&1 | head -50`).
   Наиболее вероятные причины: нет `ton/ton-types.h` на include path | нет `td/utils/misc.h` | `td::Bits256::as_slice()` не компилируется.
3. Если нет `clang-21` — попробовать `clang-20` или `clang` из пакетов по умолчанию.
4. Если нет `libjemalloc-dev` — убрать флаг из cmake (или `-DTON_USE_JEMALLOC=OFF`).
