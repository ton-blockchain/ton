...
static constexpr void const* continuation_1 = &tags[1];
static constexpr void const* continuation_2 = &tags[2];
...
TEST(FFIEventLoop, MultiplePuts) {
  ...
  EXPECT(result1.has_value() && result1->value == continuation_0);
  EXPECT(result2.has_value() && result2->value == continuation_1);
  EXPECT(result3.has_value() && result3->value == continuation_2);
...
TEST(FFIEventLoop, ConcurrentPuts) {
  ...
  std::set<void*> received;
  for (int i = 0; i < num_threads * puts_per_thread; ++i) {
    auto result = wait_for_continuation(loop);
    received.insert(const_cast<void*>(result.value));
  }
...
TEST(FFIEventLoop, BackgroundThreadFlow) {
  ...
  std::thread background([&]() {
    while (background_running) {
      auto result = loop.wait(0.01);
      if (result.has_value()) {
        received.push_back(const_cast<void*>(result->value));
      }
    }
  });
