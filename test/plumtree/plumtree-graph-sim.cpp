...
int main(int argc, char **argv) {
  ...
  if (shard_count == 0 || shard_count > validators.size()) {
  ...
  std::vector<std::size_t> shard_roots;
  shard_roots.reserve(shard_count);
  for (td::uint32 local_index = 0; local_index < shard_count; ++local_index) {
    shard_roots.push_back(validators[local_index]);
  }
  auto expected_delivery = reachable_nodes(out_neighbours, shard_roots);
  ...
  opts.plumtree_fec_options_.tree_slots_ = 500;
  ...
  std::cout << "Plumtree graph simulation: nodes=" << graph.nodes.size() << ", validators=" << validators.size()
            << ", shard=" << shard_count << ", expected=" << expected_delivered
            << ", unreachable=" << (graph.nodes.size() - expected_delivered)
            << ", broadcasts=" << settings.broadcast_count
  ...
  for (td::uint32 broadcast_index = 0; broadcast_index < settings.broadcast_count; ++broadcast_index) {
    ...
    std::cout << std::right << std::setw(4) << broadcast_index << " " << std::setw(5) << delivered << "/" << std::left
              << std::setw(5) << expected_delivered << std::right << " " << std::setw(8) << format_ms(p90_ms) << " "
              << std::setw(8) << format_ms(p95_ms) << " " << std::setw(8) << format_ms(p99_ms) << " " << std::setw(8)
              << format_ms(last_decoded_ms) << " " << std::setw(10)
              << format_bytes(traffic_summary.average_received_bytes) << " " << std::setw(10)
