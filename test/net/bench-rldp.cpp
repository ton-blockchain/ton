...
struct Config {
  ...
  bool no_admission_limits = false;
  bool transport_ceiling = false;
  bool legacy_no_limits = false;
...
// --no-admission-limits lifts only the per-peer-address admission limits. A benchmark drives every
// connection from one address, so the defaults (10 new connections per 0.2s, 1000 concurrent)
// throttle the load generator rather than the code under test. Stream credit stays at the production
// default so the cross-library suite compares the same 4096-stream window; --transport-ceiling is a
// separate, explicitly labelled single-stack ceiling mode.
...
Node make_node(const Config& config, const std::string& db_root, ton::PrivateKey private_key, char identity_tag = 'C',
  ...
  if (config.prometheus_port) {
    ...
    td::actor::send_closure(node.exporter, &ton::PrometheusExporter::ready);
  }
  return node;
}
 ...
 private:
  ...
  void on_response(td::uint32 idx, td::Result<td::BufferSlice> R) {
    ...
    if (R.is_error()) {
      ...
      LOG_CHECK(r.size() == config_.response_size && r[0] == 'R') << "Query " << idx << ": bad response";
      received_++;
    }
...
int main(int argc, char* argv[]) {
  ...
  auto disable_admission_limits = [&]() {
    config.no_admission_limits = true;
    return td::Status::OK();
  };
  p.add_checked_option('\0', "no-admission-limits", "disable per-address connection rate/flood limits (bench only)",
                       disable_admission_limits);
  p.add_checked_option('\0', "no-limits",
                       "deprecated: disable admission limits and retain the legacy QUIC transport ceiling", [&]() {
                         config.no_admission_limits = true;
                         config.legacy_no_limits = true;
                         return td::Status::OK();
                       });
  p.add_checked_option('\0', "transport-ceiling",
                       "raise QUIC bidi/uni stream credit to 65536 (bench only; not cross-library parity)", [&]() {
                         config.transport_ceiling = true;
                         return td::Status::OK();
                       });
  ...
  if (config.legacy_no_limits && config.protocol == Protocol::quic) {
    config.transport_ceiling = true;
  }
  if (config.transport_ceiling && config.protocol != Protocol::quic) {
    LOG(FATAL) << "--transport-ceiling is only meaningful with --quic";
  }
