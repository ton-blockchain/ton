...
Environment: BENCH, GO_BENCH, QUINN_BENCH, EXTRA, THREADS (default 1),
             ...
             TON_TRANSPORT_CEILING (default 0; TON QUIC only, raises stream credit to 65536).
USAGE
}
...
CC="${CC:-cubic}"
TON_TRANSPORT_CEILING="${TON_TRANSPORT_CEILING:-0}"
TYPE=query
IMPLEMENTATION=ton
PROTO=quic
CLIENTS=8
...
case "$TON_TRANSPORT_CEILING" in
  0|1) ;;
  *) echo "TON_TRANSPORT_CEILING must be 0 or 1" >&2; exit 2 ;;
esac
if [ "$TON_TRANSPORT_CEILING" -eq 1 ] && { [ "$IMPLEMENTATION" != ton ] || [ "$PROTO" != quic ]; }; then
  echo "TON_TRANSPORT_CEILING=1 is only valid with -I ton -P quic" >&2
  exit 2
fi
...
STREAM_CREDIT_PROFILE=NA
if [ "$PROTO" = quic ]; then
  if [ "$IMPLEMENTATION" = ton ]; then
    STREAM_CREDIT_PROFILE=parity-4096
    [ "$TON_TRANSPORT_CEILING" -eq 0 ] || STREAM_CREDIT_PROFILE=ceiling-65536
  elif [ "$NATIVE_TUNED" -eq 1 ]; then
    STREAM_CREDIT_PROFILE=parity-4096
  else
    STREAM_CREDIT_PROFILE=library-default
  fi
fi
DEFAULT_LABEL="$IMPLEMENTATION-$PROTO-$TYPE-$TOPOLOGY-${SIZE}x${RESPONSE_SIZE}b"
if [ "$STREAM_CREDIT_PROFILE" != NA ]; then
  DEFAULT_LABEL="$DEFAULT_LABEL-$STREAM_CREDIT_PROFILE"
fi
...
case "$(uname -s)" in
  Linux) HOST_UDP_DROP_SOURCE=proc-net-snmp-udp-rcvbuferrors ;;
  Darwin) HOST_UDP_DROP_SOURCE=netstat-udp-full-socket-buffers ;;
  *) HOST_UDP_DROP_SOURCE=unsupported ;;
esac
{
  printf 'field\tvalue\n'
  ...
  printf 'native_tuned\t%s\nstream_credit_profile\t%s\ncongestion_control\t%s\n' \
    "$RECORDED_TUNED" "$STREAM_CREDIT_PROFILE" "$RECORDED_CC"
  printf 'connect_inflight\t%s\nconnect_rate\t%s\nextra\t%s\n' \
    "$CONNECT_INFLIGHT" "$CONNECT_RATE" "${EXTRA:-}"
  printf 'host_udp_receive_drop_source\t%s\n' "$HOST_UDP_DROP_SOURCE"
  printf 'host\t%s\n' "$(uname -a)"
} > "$OUTDIR/manifest.tsv"
...
scrape_group() {
  local base="$1" count="$2" output="$3" deadline="${4:-$(deadline_after 260)}" body ok timeout attempt
  : > "$output"
  for i in $(seq 0 $((count - 1))); do
    ok=0
    for ((attempt = 1; attempt <= 50; ++attempt)); do
      timeout=$(remaining "$deadline" 5) || break
      if body=$(curl -sf --max-time "$timeout" "http://127.0.0.1:$((base + i))/metrics"); then
        printf '%s\n' "$body" >> "$output"
        ok=1
        break
      fi
      [ "$attempt" -lt 50 ] || break
      timeout=$(remaining "$deadline" 0.2) || break
      sleep "$timeout"
    done
    if [ "$ok" -ne 1 ]; then
...
deadline_after() { awk -v now="$(now)" -v delay="$1" 'BEGIN {printf "%.6f\n", now + delay}'; }
remaining() {
  awk -v now="$(now)" -v deadline="$1" -v limit="$2" \
    'BEGIN {left = deadline - now; if (left < 0.001) exit 1; printf "%.6f\n", (left < limit ? left : limit)}'
}
...
host_udp_receive_drops() {
  case "$(uname -s)" in
    Linux)
      [ -r /proc/net/snmp ] || { printf 'NA\n'; return; }
      awk '
        $1 == "Udp:" && !header {
        ...
        $1 == "Udp:" && header {
          if ("RcvbufErrors" in column) {
            print $(column["RcvbufErrors"])
            found = 1
          }
          exit
        }
      ...
      ;;
    Darwin)
      command -v netstat >/dev/null 2>&1 || { printf 'NA\n'; return; }
...
TON_TUNING=(--no-admission-limits)
if [ "$TON_TRANSPORT_CEILING" -eq 1 ]; then TON_TUNING+=(--transport-ceiling); fi

READY_DEADLINE=$(deadline_after "$READY_TIMEOUT")
for i in $(seq 0 $((SERVERS - 1))); do
  if [ "$IMPLEMENTATION" = ton ]; then
    (cd "$OUTDIR" && exec "$RUNNER" --server "--$PROTO" "${TON_TUNING[@]}" --server-id "$i" \
...
scrape_group "$SPROM" "$SERVERS" /dev/null "$READY_DEADLINE" || {
  echo "a server exporter did not start" >&2; exit 1;
}
...
for i in $(seq 1 "$CLIENTS"); do
  if [ "$IMPLEMENTATION" = ton ]; then
    (cd "$OUTDIR" && exec "$RUNNER" --client "--$PROTO" "${TON_TUNING[@]}" --client-id "$i" \
...
scrape_group "$CPROM" "$CLIENTS" /dev/null "$READY_DEADLINE" || {
  echo "a client exporter did not start" >&2; exit 1;
}
READY=0
EXPECTED_READY=$CLIENTS
if [ "$IMPLEMENTATION" != ton ]; then EXPECTED_READY=$TOPOLOGY_PEERS; fi
while remaining "$READY_DEADLINE" 0.5 >/dev/null; do
  if [ "$IMPLEMENTATION" = ton ]; then
    READY=$(grep -l "Ready: 1" "$OUTDIR"/client-*.log 2>/dev/null | wc -l | tr -d ' ')
    [ "$READY" -eq "$CLIENTS" ] && break
  elif scrape_group "$CPROM" "$CLIENTS" "$OUTDIR/readiness-client.txt" "$READY_DEADLINE" && \
       scrape_group "$SPROM" "$SERVERS" "$OUTDIR/readiness-server.txt" "$READY_DEADLINE"; then
  ...
  READY_WAIT=$(remaining "$READY_DEADLINE" 0.5) || break
  sleep "$READY_WAIT"
done
...
HOST_UDP_RECEIVE_DROPS0=$(host_udp_receive_drops)
if [ "$SIDE" = sender ]; then
...
HOST_UDP_RECEIVE_DROPS1=$(host_udp_receive_drops)
...
HOST_UDP_RECEIVE_DROPS=NA
if [[ "$HOST_UDP_RECEIVE_DROPS0" =~ ^[0-9]+$ && "$HOST_UDP_RECEIVE_DROPS1" =~ ^[0-9]+$ ]] && \
   [ "$HOST_UDP_RECEIVE_DROPS1" -ge "$HOST_UDP_RECEIVE_DROPS0" ]; then
  HOST_UDP_RECEIVE_DROPS=$((HOST_UDP_RECEIVE_DROPS1 - HOST_UDP_RECEIVE_DROPS0))
fi
{
  printf 'host_udp_receive_drops_start\t%s\n' "$HOST_UDP_RECEIVE_DROPS0"
  printf 'host_udp_receive_drops_end\t%s\n' "$HOST_UDP_RECEIVE_DROPS1"
  printf 'host_udp_receive_drops_delta\t%s\n' "$HOST_UDP_RECEIVE_DROPS"
} >> "$OUTDIR/manifest.tsv"
...
python3 "$(dirname "$0")/report.py" --dir "$OUTDIR" --implementation "$IMPLEMENTATION" \
  ...
  --stream-credit-profile "$STREAM_CREDIT_PROFILE" --congestion-control "$REPORT_CC" \
  ...
  --driver-errors "$DRIVER_ERRORS" --host-udp-receive-drops "$HOST_UDP_RECEIVE_DROPS" \
