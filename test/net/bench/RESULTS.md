...
These rows predate the admission/stream-credit split. TON QUIC's legacy `--no-limits` raised stream
credit to 65,536 while the native parity profiles used 4,096, so the cross-library saturation rows
are not stream-credit-parity comparisons. Current suite runs keep every parity profile at 4,096;
regenerate this table before making a cross-library saturation claim from it.
...
ack+PING liveness rule at sub-millisecond srtt, gone at real RTT. The historical saturation row used
the lifted stream-credit window (now `TON_TRANSPORT_CEILING=1`); the default 4096 streams cap one
connection at roughly 80k msg/s at this RTT.
