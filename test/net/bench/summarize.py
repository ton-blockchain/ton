...
def median(rows, field):
    """Return the median field value across successful repetitions only."""
    values = []
    for row in rows:
...
def main():
    ...
    for row in rows:
        key = (
            ...
            row["implementation"],
            row["protocol"],
            row.get("measured_side", "unknown"),
            row.get("stream_credit_profile", "NA"),
        )
    ...
    for key in order:
        scenario, impl, proto, side, credit = key
        group = groups[key]
