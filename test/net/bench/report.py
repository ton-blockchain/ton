...
def arguments():
    ...
    parser.add_argument(
        "--stream-credit-profile",
        choices=("NA", "parity-4096", "ceiling-65536", "library-default"),
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument("--host-udp-receive-drops", required=True)
...
def main():
    ...
    host_udp_receive_drops = None
    if args.host_udp_receive_drops != "NA":
        try:
            host_udp_receive_drops = int(args.host_udp_receive_drops)
        except ValueError as error:
            message = "--host-udp-receive-drops must be a non-negative integer or NA"
            raise SystemExit(message) from error
        if host_udp_receive_drops < 0:
            raise SystemExit("--host-udp-receive-drops must be a non-negative integer or NA")
    if args.side == "sender":
    ...
    drops = 0.0
    has_drop_counters = not native or host_udp_receive_drops is not None
    drop_details: list[str] = []
    failures = 0.0
    rows: dict[str, dict[str, float]] = {}

    if native:
        if host_udp_receive_drops is None:
            missing.append("host:udp_receive_buffer_drops")
        else:
            drops = float(host_udp_receive_drops)
            if drops:
                drop_details.append(f"host-udp-receive-buffer={drops:.0f}")
