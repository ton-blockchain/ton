...
const SATURATE_WAIT_TIMEOUT: Duration = Duration::from_secs(15);

#[derive(Default)]
struct SaturateCounter {
    connection: std::sync::OnceLock<Connection>,
    received: AtomicU64,
    invalid: AtomicU64,
    target: AtomicU64,
...
impl SaturateCounter {
    ...
    fn expect(&self, target: u64) -> Option<tokio::sync::oneshot::Receiver<()>> {
        ...
        if self.invalid.load(Ordering::Relaxed) != 0 {
            return Some(rx);
        }
        *guard = Some(tx);
        // receive can cross target before observing this store. Publish the target first, then
        // recheck the count. These operations and receive's increment/load must be sequentially
        // consistent so the two threads cannot both miss each other's update.
        self.target.store(target, Ordering::SeqCst);
        if self.received.load(Ordering::SeqCst) >= target {
    ...
    fn receive(&self, valid: bool) {
        if !valid {
            ...
            if let Some(connection) = self.connection.get() {
                connection.close(VarInt::from_u32(1), b"saturation receiver failed");
            }
            return;
        }
...
async fn wait_for(
    ...
    closed: impl std::future::Future<Output = quinn::ConnectionError>,
    timeout: Duration,
) -> Result<(), AnyError> {
    if let Some(rx) = rx {
        tokio::select! {
            result = rx => result.map_err(|_| io::Error::other("saturation receiver failed"))?,
            error = closed => return Err(error.into()),
            _ = tokio::time::sleep(timeout) => {
                return Err(io::Error::new(io::ErrorKind::TimedOut, "saturation receiver stalled").into());
            }
        }
    }
    Ok(())
...
async fn run_saturate(args: Args) -> Result<(), AnyError> {
    ...
    let warmup = counter.received.load(Ordering::Relaxed) + 1;
    send_saturate_message(&connection, &payload).await?;
    wait_received(warmup).await?;
    ...
    while start.elapsed() < duration {
        ...
        if submitted - submitted.min(received) >= MAX_RECEIVER_LAG {
            wait_received(received_at_start + submitted - RESUME_RECEIVER_LAG).await?;
        }
    }
    ...
    wait_received(received_at_start + submitted).await?;
...
fn real_main() -> Result<(), AnyError> {
    ...
    if args.workload == Workload::Saturate {
        let limit = args.duration.unwrap_or(Duration::from_secs(5)) + 2 * SATURATE_WAIT_TIMEOUT;
        return runtime.block_on(async { tokio::time::timeout(limit, run_saturate(args)).await? });
    }
    match args.role {
