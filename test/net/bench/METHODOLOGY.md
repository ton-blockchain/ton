...
| `idle-scale` | 1K, 10K and optionally 100K established application-idle sessions -> one server | all QUIC drivers available; TON large-scale validation pending | available |
...
with TON and any built native drivers; set `IMPLEMENTATIONS` to override.
...
process default is not the topology you want. Cross-library parity runs use 4,096 stream credits,
bounded flow-control windows, and 5s/15s keepalive/idle timers by default. Use
`--no-admission-limits` on the TON driver to lift only per-address admission limits without changing
stream credit. The deprecated `--no-limits` spelling preserves its historical combined behavior
for QUIC (`--no-admission-limits --transport-ceiling`) and remains an admission-only no-op for the
other transports. Set `TON_TRANSPORT_CEILING=1` only for a separately labelled TON QUIC ceiling
probe (65,536 stream credits), and set `NATIVE_TUNED=0` only when separately measuring library
defaults. Cross-library runs use CUBIC because all three implementations expose it; set `CC=bbr` or
`CC=reno` for separately labelled TON QUIC runs.
...
- target and start/end peer-session gauges, CPU/RSS from both sides, and
  application/transport/socket drops. For native drivers, the harness also samples the host UDP
  receive-buffer overflow counter around the measurement window.
...
A run is `INVALID` if required metric families or native host UDP drop counters are absent, it has
drops or transport/query failures,
...
`-c` is the number of generator processes and `-C` is the number of peer sessions each owns;
the expected count is `c * C`. Sessions share one UDP endpoint per generator; TON sessions use
distinct local ADNL identities. For fan-out, `bench.sh` sets `C` equal to the server count, with one
session per destination. Use a few generators with many sessions so process overhead does not dominate.
...
ramp, while `setup_seconds` also includes repair and the final stability gate. TON supports
multi-session generators, but has no `CONNECT_RATE` pacing or corresponding setup-time metrics.

Admission limits need explicit treatment. The benchmark's `--no-admission-limits` lifts QUIC's
per-address admission limits without changing its production 4,096-stream credit. ADNL still
rejects more than 60 recently seen peer identities from one
...
about 676 KiB/peer with TON's 4,096-stream profile but only about 43 KiB/peer with Quinn defaults.
Record the `tuned` and `stream_credit_profile` columns and do not present one profile as the other.
...
The native drivers also do not expose `SO_RXQ_OVFL`. On Linux, `bench.sh` snapshots
`/proc/net/snmp`'s `Udp:RcvbufErrors`; on macOS it snapshots `netstat -s -p udp`'s full-socket-buffer
drop count. These are host-wide counters, so any increase conservatively invalidates the run and the
benchmark host should be isolated from unrelated UDP traffic. A native run is invalid when neither
counter is available rather than being reported `ok` with an unchecked receive queue.
