...
func (c *saturateCounter) expect(target uint64) chan struct{} {
	...
	// recheck the count; Go's atomic operations are sequentially consistent, so the store and
	// recheck cannot both miss receive's increment and target load.
...
func (c *saturateCounter) receive(r io.Reader, expected int) {
	data, err := io.ReadAll(io.LimitReader(r, int64(expected+1)))
	if err == nil && len(data) != expected {
		err = fmt.Errorf("bad payload: got %d bytes, expected %d", len(data), expected)
	}
	if err != nil {
		c.fail(err)
		return
...
func (c *saturateCounter) accept(ctx context.Context, listener *quic.Listener, size int) {
	conn, err := listener.Accept(ctx)
	if err != nil {
		c.fail(err)
		return
...
func waitFor(ctx context.Context, ch chan struct{}) error {
	...
	case <-ctx.Done():
		return context.Cause(ctx)
	case <-timer.C:
		return fmt.Errorf("receiver stalled: %w", context.DeadlineExceeded)
	}
}
...
func runSaturate(ctx context.Context, opts options) error {
	ctx, stop := context.WithTimeout(ctx, time.Duration(opts.duration*float64(time.Second))+2*progressTimeout)
	defer stop()
	...
	c := &saturateCounter{fail: fail}
	go c.accept(ctx, listener, opts.size)
	...
	conn, err := clientTransport.Dial(ctx, serverUDP.LocalAddr(), clientTLS(), transportConfig(opts.tuned))
	if err != nil {
		return err
	}
	stopClose := context.AfterFunc(ctx, func() { _ = conn.CloseWithError(0, "benchmark stopped") })
	defer stopClose()
	...
	if err := waitFor(ctx, warmup); err != nil {
		return err
	}
	...
	for time.Since(start).Seconds() < opts.duration {
		...
		if submitted-min(submitted, received) >= maxReceiverLag {
			if err := waitFor(ctx, c.expect(receivedAtStart+submitted-resumeReceiverLag)); err != nil {
				return err
			}
		}
	}
	...
	if err := waitFor(ctx, c.expect(receivedAtStart+submitted)); err != nil {
		return err
	}
