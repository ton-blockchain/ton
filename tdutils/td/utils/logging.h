...
#define TD_LOG_IMPL(log_level, ...)                                                                    \
  do {                                                                                                 \
    if (TD_LOG_LEVEL >= static_cast<int32>(::td::Log::Level::log_level)) {                              \
      static_assert(::td::detail::is_valid_log_level(::td::Log::Level::log_level), "Invalid log level"); \
      auto &log_interface = ::td::Log::get_log_interface();                                             \
      if (log_interface) {                                                                             \
        constexpr int32 line_num = __LINE__;                                                           \
        log_interface->append(                                                                         \
            __FILE__, line_num, static_cast<int32>(::td::Log::Level::log_level),                        \
            format_string<TD_MSVC_THAN(1920)>(__VA_ARGS__));                                            \
      }                                                                                                \
    }                                                                                                  \
  } while (false)

#if TD_MSVC
#define LOG(level, ...) TD_LOG_IMPL(level, __VA_ARGS__)
#else
...
#define VLOG(level, ...)                        \
  do {                                                                 \
    if (VLOG_LEVEL >= static_cast<int32>(::td::Log::Level::level)) {    \
...
#define LOG_IF(level, condition, ...) \
  LOG_IMPL(level, level, condition, #condition)
