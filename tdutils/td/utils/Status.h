...
#pragma GCC diagnostic pop
  ...
  TD_WARN_UNUSED_RESULT Result<T> clone() const requires (Cloneable<T>) {
    if (is_ok()) {
      return Result<T>(ok().clone());
    }
    return error().clone();
  }
  TD_WARN_UNUSED_RESULT Result<T> clone() const requires (!Cloneable<T> && std::is_copy_constructible_v<T>) {
    if (is_ok()) {
      return Result<T>(ok());
    }
    return error().clone();
